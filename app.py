...
if uploaded_files:
    if st.button("🚀 Run Complete Sustainability Analysis", type="primary", use_container_width=True):
        with st.spinner("🤖 AI Agent is analyzing your data... This may take a few moments."):
            ...
            # Step 2: Analysis
            status_text.text("🔬 Step 2/5: Performing sustainability calculations...")
            results = st.session_state.analyzer.analyze_all(uploaded_files, analysis_type)
            progress_bar.progress(40)
            
            # Step 3: Image analysis
            status_text.text("🖼️ Step 3/5: Analyzing images...")
            image_analyzer = ImageAnalyzer()
            image_results = image_analyzer.analyze_images([f for f in uploaded_files if f.type in ['image/jpeg', 'image/png', 'image/jpg']])
            progress_bar.progress(60)
            
            # Step 4: Text classification
            status_text.text("📄 Step 4/5: Classifying text against GRI standards...")
            text_classifier = TextClassifier()
            text_results = text_classifier.classify_documents([f for f in uploaded_files if f.type in ['application/pdf', 'application/vnd.openxmlformats-officedocument.wordprocessingml.document']])
            progress_bar.progress(80)
            
            # Step 5: Predictions
            status_text.text("🔮 Step 5/5: Generating forecasts and insights...")
    ...
    if st.session_state.results:
        ...
        with tab1:
            ...
            with col1:
                if 'energy_data' in results_data['analysis']:
                    fig = px.line(results_data['analysis']['energy_data'], title="Energy Consumption Trend")
                    st.plotly_chart(fig, use_container_width=True)
            
            with col2:
                if 'emissions_data' in results_data['analysis']:
                    fig = px.bar(results_data['analysis']['emissions_data'], title="Emissions by Source")
                    st.plotly_chart(fig, use_container_width=True)
            ...
            if results_data['image_analysis']:
                for img_res in results_data['image_analysis']:
                    ...
                    with col2:
                        ...
                        st.markdown(f"**Detected Objects:** {', '.join(img_res['detected_objects'][:5])}")
        ...
        with tab3:
            ...
            if results_data['text_classification']:
                for doc in results_data['text_classification']:
                    ...
                    # Key phrases
                    st.markdown("**🔑 Key Sustainability Phrases Detected:**")
                    st.write(doc['key_phrases'][:10])
            ...
            if results_data['predictions']:
                ...
                with col1:
                    ...
                    **🎯 Short-term (2025-2026):**
                    ...
                    - Implement LED lighting retrofits
                    - Start water recycling pilot
                    """)
                with col2:
                    st.markdown("""
                    **🚀 Long-term (2027-2030):**
        ...
        with tab5:
            ...
            report_format = st.radio("Report Format", ["PDF Report", "Excel Summary", "Both"])
